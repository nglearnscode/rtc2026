...
// check-in confirmation DM. Fill in real UofT emails below — nothing here
// is sent anywhere until you also fill in TEAMS_WEBHOOKS below. Kept as a
// separate table from SAMPLE_ROSTERS on purpose, so nothing about existing
// check-in/roster logic (which matches on name strings already stored in
// Firestore) has to change.
const ROSTER_EMAILS = {
  Northside: {
    "Juna Abutaha": "REPLACE_WITH_EMAIL", "Suryansh Baichoo": "REPLACE_WITH_EMAIL", "Cielo Barrera Amacifuen": "REPLACE_WITH_EMAIL", "Xavier Blache": "REPLACE_WITH_EMAIL", "Saif Cheema": "REPLACE_WITH_EMAIL", "Rayanna Clarke": "REPLACE_WITH_EMAIL", "Mackenzie Geith": "REPLACE_WITH_EMAIL", "Javonte Hunter": "REPLACE_WITH_EMAIL", "Eilin Kulinich": "REPLACE_WITH_EMAIL", "Sonakshi Mitra": "REPLACE_WITH_EMAIL", "Georgina Murray": "REPLACE_WITH_EMAIL", "Adegheosa Omorogiuwa": "REPLACE_WITH_EMAIL", "Shahzaib Sheikh": "REPLACE_WITH_EMAIL", "Khue Tran": "REPLACE_WITH_EMAIL",
  },
  Southside: {
    "Amelia Alam": "REPLACE_WITH_EMAIL", "Spencer Brodie": "REPLACE_WITH_EMAIL", "Dilni Dissanayake": "REPLACE_WITH_EMAIL", "Jenny Dong": "REPLACE_WITH_EMAIL", "Irha Hassan": "REPLACE_WITH_EMAIL", "Munkhzul Jargalsaikhan": "REPLACE_WITH_EMAIL", "Tishma Joarder": "REPLACE_WITH_EMAIL", "Farhada Khaled": "REPLACE_WITH_EMAIL", "Mairin McConnell": "REPLACE_WITH_EMAIL", "Helya Rassouli": "REPLACE_WITH_EMAIL", "Karisa Sol-Edeigba": "REPLACE_WITH_EMAIL", "Kels Themens": "REPLACE_WITH_EMAIL", "Raisha Valezka": "REPLACE_WITH_EMAIL", "Ji-in Yun": "REPLACE_WITH_EMAIL", "Sofia Benard": "REPLACE_WITH_EMAIL", "Aisling MacQuarrie": "REPLACE_WITH_EMAIL", "Maria Paula Garzon": "REPLACE_WITH_EMAIL",
  },
  Central: {
    "Divine Aine": "REPLACE_WITH_EMAIL", "Hoor Atique": "REPLACE_WITH_EMAIL", "Ximena Castillo Ramirez": "REPLACE_WITH_EMAIL", "Ava Jalali": "REPLACE_WITH_EMAIL", "Julia Kary": "REPLACE_WITH_EMAIL", "Oona Kauppinen": "REPLACE_WITH_EMAIL", "Rachel Rui Liang": "REPLACE_WITH_EMAIL", "Siddharth Naik": "REPLACE_WITH_EMAIL", "Daphne Nguyen": "REPLACE_WITH_EMAIL", "Mitaansh Niverthi": "REPLACE_WITH_EMAIL", "Areesh Noman": "REPLACE_WITH_EMAIL", "Senad Sadik": "REPLACE_WITH_EMAIL", "Kaitlyn Stevens": "REPLACE_WITH_EMAIL", "Jacelynn Su": "REPLACE_WITH_EMAIL", "Krish Thakur": "REPLACE_WITH_EMAIL",
  },
  NRB: {
    "Katia Al Shehadeh": "REPLACE_WITH_EMAIL", "Riana Banks": "REPLACE_WITH_EMAIL", "Ava Boston": "REPLACE_WITH_EMAIL", "Aryan Garg": "REPLACE_WITH_EMAIL", "Tia Girolametto": "REPLACE_WITH_EMAIL", "Sabea Larson": "REPLACE_WITH_EMAIL", "Sarah McGinn": "REPLACE_WITH_EMAIL", "Presley McLeod": "REPLACE_WITH_EMAIL", "Emma-Marie Meyer": "REPLACE_WITH_EMAIL", "Giulia Nuvoloni": "REPLACE_WITH_EMAIL", "Semilore Olafare": "REPLACE_WITH_EMAIL", "Princess Olawale": "REPLACE_WITH_EMAIL", "Sreyasi Rahman": "REPLACE_WITH_EMAIL", "Aurora Thompson": "REPLACE_WITH_EMAIL", "Gio Williams": "REPLACE_WITH_EMAIL",
  },
  RSA: {
    "Divva Achpilya": "REPLACE_WITH_EMAIL", "Reese Cameron": "REPLACE_WITH_EMAIL", "MJ Comuzzi": "REPLACE_WITH_EMAIL", "Olivia Dalla Zanna": "REPLACE_WITH_EMAIL", "Avery Harvey": "REPLACE_WITH_EMAIL", "Madeline Kelly": "REPLACE_WITH_EMAIL", "Heeje Kim": "REPLACE_WITH_EMAIL", "Karina Kosareva": "REPLACE_WITH_EMAIL", "Kartika Kumala": "REPLACE_WITH_EMAIL", "Hamad Malik": "REPLACE_WITH_EMAIL", "Ghazlia Mehdi": "REPLACE_WITH_EMAIL", "Lawrene Naval": "REPLACE_WITH_EMAIL", "Michelle Omitayo": "REPLACE_WITH_EMAIL", "Fehintoluwa Omotunde": "REPLACE_WITH_EMAIL", "Princess Owusu Ansah": "REPLACE_WITH_EMAIL", "Blake Pederson": "REPLACE_WITH_EMAIL", "Matthew Pichocki": "REPLACE_WITH_EMAIL", "Krisha Shah": "REPLACE_WITH_EMAIL", "Darsh Singh": "REPLACE_WITH_EMAIL", "Natran Tewoldemedhin": "REPLACE_WITH_EMAIL", "Kayla Viveiros": "REPLACE_WITH_EMAIL", "Akaila Wright": "REPLACE_WITH_EMAIL",
  },
  REF: {
...
function getRosterEmail(team, name) {
  const email = ROSTER_EMAILS[team] && ROSTER_EMAILS[team][name];
...
async function postToTeamsWebhook(url, payload) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // that reaches the server but gets rejected (e.g. 401 from a
      // truncated/incomplete webhook URL) resolves normally and would
      // otherwise be silently logged as a success. Surface it as a failure
      // instead, with the real status code, since that's the actual signal
      // needed to diagnose a bad URL vs. a genuine CORS block.
      debugLog(`Teams webhook REJECTED (HTTP ${response.status} ${response.statusText}): ${JSON.stringify(payload).slice(0, 90)}`);
      return;
    }
...
function sendCheckinConfirmation(name, team, sessionWindow, tier, message) {
  const email = getRosterEmail(team, name);
  if (!email) return; // no email on file for this person yet — skip quietly
...
function sendExceptionAlert(alertType, details) {
  postToTeamsWebhook(TEAMS_WEBHOOKS.exceptionAlertUrl, { alertType, ...details });
}
...
async function submitCheckIn() {
  ...
  if (tier === "late") {
    ...
    sendCheckinConfirmation(name, team, sessionWindow, tier, checkinFormState.result.message);
  } else {
    await addAttendanceRecord(name, team, sessionWindow, tier, minutesEarly);
...
function wireAll() {
  ...
  document.querySelectorAll("[data-delete-record]").forEach((el) => {
    el.onclick = () => {
      ...
      const [w, team, indexStr] = key.split("|");
      if (!window.confirm(`Delete this attendance record for ${teamDisplayName(team)}? This can't be undone.`)) return;
