...
// Internal team codes (above) are the actual data keys used everywhere —
// Firestore records, roster lookups, excused lists, dropdown values, etc.
// This mapping ONLY controls what's shown on screen, so renaming a team
// never requires touching stored data or any lookup logic.
...
const POINT_VALUES = {
  ...
  themeDayPool: 40,
  themeDayPoolFull: 80, // if every eligible person (roster minus excused) dresses on theme, the pool doubles
  socialPool: 50,
  preSessionFirst: 100,
  preSessionSecond: 60,
  preSessionThird: 30,
...
const SAMPLE_ROSTERS = {
  ...
  Central: ["Pru Feeney", "Hoor Atique", "Ximena Castillo Ramirez", "Ava Jalali", "Julia Kary", "Oona Kauppinen", "Rachel Rui Liang", "Siddharth Naik", "Daphne Nguyen", "Rufin Hsu", "Areesh Noman", "Senad Sadik", "Kaitlyn Stevens", "Jacelynn Su", "Krish Thakur"],
  ...
  RSA: ["Divva Achpilya", "Reese Cameron", "MJ Comuzzi", "Olivia Dalla Zanna", "Avery Harvey", "Madeline Kelly", "Heeje Kim", "Karina Kosareva", "Kartika Kumala", "Hamad Malik", "Lawrene Naval", "Michelle Omitayo", "Fehintoluwa Omotunde", "Princess Owusu Ansah", "Blake Pederson", "Matthew Pichocki", "Krisha Shah", "Darsh Singh", "Natran Tewoldemedhin", "Kayla Viveiros", "Akaila Wright"],
...
const ROSTER_SIZES = { Northside: 14, Southside: 17, Central: 15, NRB: 15, RSA: 21, REF: 21, PF: 8 };

// ===================== TEAMS NOTIFICATIONS (fill in before enabling) =====================
// Per-person email lookup, used ONLY to send each RSS member their own
// check-in confirmation DM. Real emails are NOT stored in this file — this
// file is served publicly by GitHub Pages, so anyone could view-source it.
// Instead, emails live in a Firestore document (collection "rosterEmails",
// doc "current") and are loaded once at boot into rosterEmailsCache below.
// See roster-emails-seed.html for the one-time local tool used to write
// that document — that seed file should NEVER be committed to GitHub.
let rosterEmailsCache = {}; // populated by loadRosterEmails() during boot()

function rosterEmailsDocRef() {
  const { db, doc } = window.__firebase;
  return doc(db, "rosterEmails", "current");
}

async function loadRosterEmails() {
  try {
    const { getDoc } = window.__firebase;
    const snap = await getDoc(rosterEmailsDocRef());
    rosterEmailsCache = snap.exists() ? (snap.data().byTeam || {}) : {};
    debugLog(`loadRosterEmails -> ${snap.exists() ? "loaded" : "no doc found yet (run the seed tool)"}`);
  } catch (e) {
    rosterEmailsCache = {};
    debugLog(`loadRosterEmails FAILED (non-blocking, confirmations just won't send): ${e.message || e}`);
  }
}

// ===================== GLOBAL APP SETTINGS =====================
// A small set of global, cross-day toggles — currently just the public
// leaderboard blackout for the final stretch of the competition. Lives in
// its own tiny Firestore doc since it isn't scoped to any one day. SRSS-only
// to change, but loaded on every boot (public included), since the public
// check-in phones are exactly who need to know whether to hide the board.
function appSettingsDocRef() {
  const { db, doc } = window.__firebase;
...
async function loadAppSettings() {
  try {
    ...
    state.leaderboardHidden = snap.exists() ? !!snap.data().leaderboardHidden : false;
    debugLog(`loadAppSettings -> leaderboardHidden: ${state.leaderboardHidden}`);
  } catch (e) {
    state.leaderboardHidden = false; // fail open — never accidentally hide the board due to a load error
    debugLog(`loadAppSettings FAILED (non-blocking, defaulting to leaderboard visible): ${e.message || e}`);
  }
}
...
async function setLeaderboardHidden(hidden) {
  ...
  try {
    await setDoc(appSettingsDocRef(), { leaderboardHidden: hidden });
    state.leaderboardHidden = hidden;
    debugLog(`setLeaderboardHidden -> ${hidden}`);
  } catch (e) {
    debugLog(`setLeaderboardHidden FAILED: ${e.message || e}`);
    window.alert("Couldn't save that change — check your connection and try again.");
  }
  render();
}

function getRosterEmail(team, name) {
  const email = rosterEmailsCache[team] && rosterEmailsCache[team][name];
  return email && email !== "REPLACE_WITH_EMAIL" ? email : null;
}


// Paste the two Workflow webhook URLs here once set up in Teams (see the
// setup guide). Left as placeholders, nothing fires until both are filled
// in — postToTeamsWebhook() below checks for the placeholder text and
// silently no-ops if it's still present, so it's safe to deploy before
// these are ready.
// TESTING SAFETY VALVE — set this to your own email to redirect EVERY
// confirmation DM here instead of whoever's real email is on file,
// regardless of which real person you test-check-in as. This is what lets
...
async function postToTeamsWebhook(url, payload) {
  if (!url || url.startsWith("PASTE_")) return; // not configured yet — no-op
  try {
    ...
    if (!response.ok) {
      // fetch() only throws on a genuine network-level failure — a request
      // that reaches the server but gets rejected (e.g. a schema mismatch)
      // resolves normally and would otherwise be silently logged as a
      // success. Read the actual response body too — that's where the real
      // rejection reason lives, not just the status code.
      let bodyText = "";
      try { bodyText = await response.text(); } catch { /* ignore */ }
      debugLog(`Teams webhook REJECTED (HTTP ${response.status} ${response.statusText}): ${bodyText.slice(0, 300)}`);
      return;
    }
    debugLog(`Teams webhook sent: ${JSON.stringify(payload).slice(0, 90)}`);
  } catch (e) {
    // Never let a Teams notification failure affect check-in itself — most
    // likely cause here is the browser's CORS policy blocking the request;
    // see the setup guide for how to confirm/work around this.
    debugLog(`Teams webhook FAILED (non-blocking): ${e.message || e}`);
  }
}

function sendCheckinConfirmation(name, team, sessionWindow, tier, message, sessionName) {
  const realEmail = getRosterEmail(team, name);
  if (!realEmail) return; // no email on file for this person yet — skip quietly
  const recipientEmail = TESTING_OVERRIDE_EMAIL || realEmail;
  if (TESTING_OVERRIDE_EMAIL) {
    debugLog(`TEST MODE ON: confirmation for ${name} redirected to ${TESTING_OVERRIDE_EMAIL} (their real email was withheld)`);
  }
  postToTeamsWebhook(TEAMS_WEBHOOKS.checkinConfirmationUrl, {
    recipientEmail,
    recipientName: name,
    ...
    tier,
    message,
    sessionName: sessionName || "", // e.g. "Fire Safety Training" — blank if SRSS never named this session
  });
}

function sendExceptionAlert(alertType, details) {
  // The flow's "Post card" action errored with "Property 'type' must be
  // 'AdaptiveCard'" when sent the full message/attachments envelope —
  // meaning it's bound to the whole trigger body directly as the card, not
  // to attachments[0].content. Send the bare card itself instead.
...
function makeEmptyDay(dayIndex, carriedRosterSizes, status) {
  ...
  return {
    ...
    themeName,
    // "draft" — created ahead of time, fully editable, but NOT what public
    // check-in points to yet. "live" — the one real check-in flows to right
    // now. "closed" — a past day, done for good. Exactly one day should be
    // "live" at a time; everything else is draft or closed.
    status: status || "draft",
    ...
    preSessionPM: null,
    arbitraryAwards: [],
    rosterSizes: carriedRosterSizes ? { ...carriedRosterSizes } : { ...ROSTER_SIZES },
...
// An excused person's on-theme toggle should never count toward the team's
// score — same principle as Attendance, where excused status blocks
// counting entirely, not just shrinks a denominator. Without this, someone
// could be marked BOTH excused (shrinking the roster denominator) AND
// on-theme (counting in the numerator), letting the numerator exceed the
// denominator and produce MORE than the intended pool maximum — exactly
// how a team ended up with 87 points on an 80-point-max category.
function effectiveOnThemeCount(onTheme, excusedSet) {
  return onTheme.filter((e) => !excusedSet.has(e.name)).length;
}
...
// Theme Day pool doubles (40 -> 80) only if EVERY eligible person (roster
// minus excused) actually dressed on theme — a team-wide all-or-nothing
// bonus, not a per-person tier like Attendance's early/on-time.
function themeDayPoolFor(onThemeCount, effectiveRoster) {
  return onThemeCount >= effectiveRoster ? POINT_VALUES.themeDayPoolFull : POINT_VALUES.themeDayPool;
}
...
function calculateCumulativeTotals(days) {
  ...
  days.forEach((day) => {
    ...
    day.themeDay.forEach((row) => {
      const excusedSet = excusedForThemeDay(day, row.team);
      const roster = effectiveRosterCount(day.rosterSizes[row.team], excusedSet.size);
      const onThemeCount = effectiveOnThemeCount(row.onTheme, excusedSet);
      const pool = themeDayPoolFor(onThemeCount, roster);
      totals[row.team].themeDay += onThemeCount * (pool / roster);
    });
...
// This document's whole job is to be the shared source of truth for "which
// days exist" — so it must NEVER be blindly overwritten with just this
// device's own local picture. A device with a stale/incomplete local view
// (an old tab left open, a slow sync) could otherwise silently erase other
// real days from this list — not delete their data, just hide it from
// every future page load, which is exactly what happened once already.
// Instead: read the current REAL list, merge in whatever this device knows
// about, and write back the union, inside a transaction so two devices
// doing this at once can't clobber each other either.
async function saveDayIndex(labels) {
  const { db, runTransaction } = window.__firebase;
  const ref = dayDocRef("__index__");
  await runTransaction(db, async (tx) => {
    const snap = await tx.get(ref);
    const existing = snap.exists() && Array.isArray(snap.data().labels) ? snap.data().labels : [];
    const merged = Array.from(new Set([...existing.map(Number), ...labels.map(Number)])).sort((a, b) => a - b);
    tx.set(ref, { labels: merged });
  });
}
...
function attachDayListenerForCurrentView() {
  ...
  const dayIndex = state.viewingDayIndex || (liveDay() ? liveDay().dayIndex : null);
  if (!dayIndex) return;
  try {
    unsubscribeDayListener = onSnapshot(dayDocRef(dayStorageKey(dayIndex)), (snap) => {
      if (!snap.exists()) return;
      applyRemoteDayData(snap.data());
...
const state = {
  ...
  liveCodes: {},
  // Tracks the last-known Firestore revision (__rev) of each day document
  // this device has actually seen — keyed by dayIndex. Used to detect when
  // someone else has saved a change since we last loaded, so a save never
  // silently overwrites someone else's concurrent edit.
  dayRevisions: {},
  // Global public-leaderboard blackout (e.g. for the final stretch of the
  // competition), loaded fresh every boot — see loadAppSettings().
  leaderboardHidden: false,
};

// Applies a day document as loaded FROM Firestore (via getDoc or a live
// listener) into local state, stripping the internal __rev bookkeeping
// field out of the day object itself and recording it separately. Anything
// that replaces local day data with server data should go through this, so
// state.dayRevisions never drifts out of sync with what's actually stored.
function applyRemoteDayData(remoteData) {
  if (!remoteData || typeof remoteData.dayIndex !== "number") return;
  const { __rev, ...clean } = remoteData;
  const idx = state.days.findIndex((d) => d.dayIndex === clean.dayIndex);
  if (idx !== -1) state.days[idx] = clean;
  else state.days.push(clean);
  state.dayRevisions[clean.dayIndex] = __rev || 0;
}
...
// The one day whose status is "live" — the day public check-in flows to,
// and the default admin view when nobody's explicitly browsing a specific
// day. Falls back to the most recently created day if, for some reason,
// nothing is marked live yet (e.g. very first boot, before any explicit
// go-live action has ever happened).
function liveDay() {
  // Defensive against exactly one bad state that's happened before: more
  // than one day marked "live" at once (e.g. a device with an incomplete
  // view of which days exist mistakenly concluding an old day was current).
  // Array.find() would silently return whichever comes first — usually the
  // OLDEST day, since days are created in order — which is backwards from
  // what's actually wanted. Picking the highest dayIndex among any "live"
  // matches means the most sensible, most recent one always wins, even in
  // this invalid state, rather than QR codes and check-in silently pointing
  // at a stale earlier day.
  const liveMatches = state.days.filter((d) => d.status === "live");
  if (liveMatches.length > 0) {
    return liveMatches.reduce((a, b) => (b.dayIndex > a.dayIndex ? b : a));
  }
  return state.days[state.days.length - 1];
}

// Always the real, current LIVE day — never affected by whichever day an
// SRSS happens to be browsing in the admin dashboard, and never a draft day
// prepped ahead of time but not yet switched to. The public check-in flow
// must always use this, not today(), so a real RSS check-in can never
// accidentally get recorded against a past OR future day just because an
// admin was reviewing/prepping it in another tab on the same device.
function latestDay() {
  return liveDay();
}
...
// ===================== BOOT =====================
// One-time migration for days saved before the draft/live/closed status
// field existed. Runs harmlessly every boot but only actually does
// anything the first time — once a real "live" day is persisted, this
// finds it immediately and exits without touching anything.
async function migrateLegacyDayStatuses() {
  if (state.days.some((d) => d.status === "live")) return; // already migrated, or a fresh install
  if (state.days.length === 0) return;
  const maxIndex = Math.max(...state.days.map((d) => d.dayIndex));
  state.days.forEach((d) => { d.status = d.dayIndex === maxIndex ? "live" : "closed"; });
  try {
    await mutateDayTransactional(maxIndex, (day) => { day.status = "live"; });
    debugLog(`Migrated legacy day data — Day ${maxIndex} marked as live (one-time, won't run again).`);
  } catch (e) {
    debugLog(`migrateLegacyDayStatuses FAILED (non-blocking, will retry next load): ${e.message || e}`);
  }
}

// Corrects an invalid state that's happened before: more than one day
// marked "live" at once, usually from a device with an incomplete view of
// which days exist (e.g. the __index__ corruption bug) mistakenly
// concluding an old day was current. liveDay() already defends against
// this by picking the highest-dayIndex match, but that's a local
// workaround — this actually fixes the underlying data in Firestore, so
// every device converges on the same correct answer instead of each one
// silently working around the same bad state indefinitely.
async function reconcileLiveDayStatus() {
  const liveMatches = state.days.filter((d) => d.status === "live");
  if (liveMatches.length <= 1) return; // nothing to fix
  const correctLiveIndex = Math.max(...liveMatches.map((d) => d.dayIndex));
  const toClose = liveMatches.filter((d) => d.dayIndex !== correctLiveIndex);
  for (const day of toClose) {
    try {
      await mutateDayTransactional(day.dayIndex, (d) => { d.status = "closed"; });
      debugLog(`Reconciled duplicate "live" status — Day ${day.dayIndex} marked closed (Day ${correctLiveIndex} is the real live day).`);
    } catch (e) {
      debugLog(`reconcileLiveDayStatus FAILED for Day ${day.dayIndex} (non-blocking): ${e.message || e}`);
    }
  }
}
...
async function boot() {
  if (TESTING_OVERRIDE_EMAIL) {
    debugLog(`🧪 TEST MODE IS ON — all confirmation DMs redirect to ${TESTING_OVERRIDE_EMAIL}, not real recipients. Set TESTING_OVERRIDE_EMAIL to "" in app.js before going live.`);
  } else {
    debugLog(`✅ Test mode is OFF — confirmation DMs route to real roster emails.`);
  }
  await loadRosterEmails();
  ...
  try {
    ...
    if (!indices || indices.length === 0) {
      ...
      const day1 = makeEmptyDay(1, null, "live");
      await saveDay(dayStorageKey(1), day1);
      await saveDayIndex([1]);
      state.days = [day1];
      state.dayRevisions[1] = 0; // freshly created, matches what saveDay just wrote (no __rev yet — treated as rev 0)
    } else {
      const loaded = await Promise.all(indices.map((i) => loadDay(dayStorageKey(i))));
      state.days = [];
      loaded.forEach((d, i) => {
        if (d) applyRemoteDayData(d);
        else { const empty = makeEmptyDay(indices[i]); state.days.push(empty); state.dayRevisions[indices[i]] = 0; }
      });
      await migrateLegacyDayStatuses();
      await reconcileLiveDayStatus();
    }
    state.loading = false;
...
function renderPublicLeaderboardHtml() {
  if (state.leaderboardHidden) {
    return `
      <div class="card">
        <p class="result-icon" style="text-align:center">🏆</p>
        <h2 class="result-title" style="text-align:center">Standings are under wraps!</h2>
        <p class="result-message" style="text-align:center">The leaderboard is hidden for the final stretch of RTC — check back for the big reveal. Keep showing up and earning points!</p>
      </div>`;
  }
        ...
        <tbody>
          ${rows.map((row, i) => `
            <tr class="${i === 0 ? "tr-first" : ""}">
              <td>${i + 1}</td>
              <td style="font-weight:700">${escapeHtml(teamDisplayName(row.team))}</td>
    ...
    return `
      <div class="form-shell">
        <div class="form-card">
          ...
          <p class="result-detail">${escapeHtml(checkinFormState.name)} · ${escapeHtml(teamDisplayName(checkinFormState.team))} · ${checkinFormState.window}</p>
        </div>
      </div>`;
  }
      ...
      <div class="form-card">
        ...
        ${teamOptions.length > 1 || !isDeepLink ? `
          ...
          <select id="checkin-team">
            ...
            ${teamOptions.map((t) => `<option value="${t}" ${checkinFormState.team === t ? "selected" : ""}>${escapeHtml(teamDisplayName(t))}</option>`).join("")}
          </select>` : ""}
...
async function submitCheckIn() {
  ...
  // point contribution and confuse the attendance record). This is a fast
  // pre-check against local data — the authoritative check happens
  // atomically with the write itself, in attemptAddAttendanceRecord below.
  ...
  if (alreadyCheckedIn) {
    ...
    sendExceptionAlert("duplicate", { name, team: teamDisplayName(team), window: sessionWindow, day: day.label, detail: "Attempted to check in a second time this session" });
  ...
  if (excusedList.includes(name)) {
    ...
    sendExceptionAlert("excused-conflict", { name, team: teamDisplayName(team), window: sessionWindow, day: day.label, detail: "Tried to check in while marked excused for this session" });
  ...
  // The per-group session name SRSS typed in Session Setup (e.g. "Fire
  // Safety Training") — blank if never set, or if this was the manual
  // admin-testing path with no real timeKey.
  const sessionName = timeKey && config.sessionNames ? config.sessionNames[timeKey] || "" : "";

  // The early duplicate/excused checks above are a fast pre-check against
  // possibly slightly-stale local data — fine for immediately catching the
  // common case, but NOT sufficient on their own: two submissions arriving
  // within the same moment could both pass that check before either has
  // written anything. attemptAddAttendanceRecord() re-checks both
  // conditions again, atomically, as part of the actual write — that result
  // is the one that's actually trusted.
  const outcome = await attemptAddAttendanceRecord(day.dayIndex, name, team, sessionWindow, tier, minutesEarly);
  ...
  if (tier === "late") {
    checkinFormState.result = { status: "late", message: "You're checked in, but this session's window has passed - no points awarded. Try coming early next time to earn your team some points!" };
    sendCheckinConfirmation(name, team, sessionWindow, tier, checkinFormState.result.message, sessionName);
  } else {
    ...
    checkinFormState.result = {
      ...
      message: !rtcActive
        ...
        : "Checked in on time. Your team earns standard points for this one. Try coming early next time to earn your team double points!",
    };
    sendCheckinConfirmation(name, team, sessionWindow, tier, checkinFormState.result.message, sessionName);
    if (tier === "early" && minutesEarly >= SUSPICIOUS_EARLY_MINUTES) {
      sendExceptionAlert("suspicious-early", { name, team: teamDisplayName(team), window: sessionWindow, day: day.label, detail: `Checked in ${Math.round(minutesEarly)} min before start — worth a glance` });
    }
  }
  render();
}
  ...
  Object.entries(groups).forEach(([timeKey, teams]) => {
    ...
    cardsHtml += `
      <div class="group-card" data-group-card="${groupKey}">
        ...
        <p class="group-card-teams">${teams.map(teamDisplayName).join(" + ")}</p>
    ...
    <div class="shell">
      ...
      <div class="day-bar">
        ...
        <select id="day-selector" class="cutoff-select">
          ${state.days.map((d) => `<option value="${d.dayIndex}" ${d.dayIndex === day.dayIndex ? "selected" : ""}>${escapeHtml(d.label)}${d.status === "live" ? " (live)" : d.status === "draft" ? " (draft — not live)" : ""}</option>`).join("")}
        </select>
        ${day.status === "draft" ? `
          <span class="not-live-badge" style="font-weight:700">📝 Draft — fully editable, but check-in isn't pointed here yet</span>
          <button class="new-day-btn" id="go-live-btn">Go Live With This Day</button>
        ` : day.status !== "live" ? `
          <span class="not-live-badge" style="font-weight:700">⚠ Viewing a past day — edits here won't affect today's live check-in</span>
        ` : ""}
        <button class="new-day-btn" id="prep-next-day-btn" style="background:transparent;color:#001E51;border:2px solid #001E51">+ Prep Next Day</button>
  ...
  if (state.adminTab !== "display") {
    html += `
      <div class="export-row">
        ...
        <span class="export-hint">Includes Leaderboard, Point Log, Rejected Check-Ins, Deleted Records, and a pure Attendance Checklist (no RTC points)</span>
      </div>
...
function renderAdminLeaderboardHtml(totals, day) {
  ...
  return `
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:10px 12px;margin-bottom:14px;border:2px solid ${state.leaderboardHidden ? "#b3422a" : "#e3ddc9"};border-radius:8px;background:${state.leaderboardHidden ? "#fff0ed" : "transparent"}">
        <div>
          <p style="font-weight:700;margin:0">${state.leaderboardHidden ? "🔒 Public leaderboard is HIDDEN" : "👁 Public leaderboard is visible"}</p>
          <p class="card-note" style="margin:2px 0 0">This only affects what RSS see — you always see full real numbers here regardless.</p>
        </div>
        <button class="submit-btn" style="background:${state.leaderboardHidden ? "#0080A2" : "#b3422a"}" id="toggle-leaderboard-visibility-btn">${state.leaderboardHidden ? "Reveal to RSS" : "Hide from RSS"}</button>
      </div>
            ...
            if (t) {
              return `
                <div class="team-time-row">
                  <span class="team-time-label">${teamDisplayName(team)}</span>
          ...
          <div style="margin-top:14px">
            ...
            ${Object.entries(groups).map(([timeKey, teams]) => `
              <div style="margin-bottom:8px">
                <label class="rank-label">${teams.map(teamDisplayName).join(" + ")}</label>
...
function renderAttendanceRecordsHtml(day) {
  ...
  TEAMS.forEach((team) => {
    ...
    html += `<div class="team-block"><p class="section-label">${teamDisplayName(team)} - ${records.length} check-in${records.length !== 1 ? "s" : ""} (expected: ${expected}${excusedList.length > 0 ? `, ${excusedList.length} excused: ${excusedList.map(escapeHtml).join(", ")}` : ""})</p>`;
    if (records.length === 0) {
...
function renderManualEntryHtml(day) {
  ...
  if (cat === "theme-day") {
    html += `<p class="card-note">Tap each individual confirmed dressed on-theme today (${day.label}). ${POINT_VALUES.themeDayPool} pts available per team, split evenly across everyone expected (roster minus excused) — and if EVERY eligible person dresses on theme, the pool doubles to ${POINT_VALUES.themeDayPoolFull} for that team.</p>`;
    day.themeDay.forEach((row) => {
      const excusedSet = excusedForThemeDay(day, row.team);
      const roster = effectiveRosterCount(day.rosterSizes[row.team], excusedSet.size);
      const onThemeCount = effectiveOnThemeCount(row.onTheme, excusedSet);
      const pool = themeDayPoolFor(onThemeCount, roster);
      const isFull = pool === POINT_VALUES.themeDayPoolFull;
      const pts = Math.round(onThemeCount * (pool / roster));
      const excusedOnThemeConflict = row.onTheme.filter((e) => excusedSet.has(e.name)).map((e) => e.name);
      html += `
        <div class="team-block">
          <p class="section-label">${teamDisplayName(row.team)} - ${row.onTheme.length}/${row.roster.length} on-theme (+${pts} pts${isFull ? " — full team, doubled!" : ""})</p>
          ${excusedOnThemeConflict.length > 0 ? `<p class="card-note" style="color:#b3422a">⚠ ${excusedOnThemeConflict.map(escapeHtml).join(", ")} marked on-theme but also excused today — not counted toward points until resolved (un-excuse them or untoggle on-theme, whichever is correct).</p>` : ""}
          <div class="team-grid">
  ...
  if (cat === "social") {
    ...
    day.socials.forEach((row) => {
      ...
      html += `
        <div class="team-block">
          <p class="section-label">${teamDisplayName(row.team)} - ${row.attendees.length}/${row.roster.length} attended (+${pts} pts)</p>
          <div class="team-grid">
            ${row.roster.map((person) => {
              ...
              ${["first", "second", "third"].map((place, pi) => `
                <div class="rank-picker">
                  ...
                  <select data-rank="${sessionKey}|${place}">
                    <option value="">—</option>
                    ${TEAMS.map((t) => `<option value="${t}" ${session[place] === t ? "selected" : ""}>${escapeHtml(teamDisplayName(t))}</option>`).join("")}
                  </select>
                </div>`).join("")}
            </div>` : ""}
        </div>`;
  ...
  if (cat === "arbitrary") {
    html += `
      ...
      <div class="form-grid">
        <div><label class="form-label">Team</label><select id="arb-team">${TEAMS.map((t) => `<option value="${t}">${escapeHtml(teamDisplayName(t))}</option>`).join("")}</select></div>
      ...
      <table><thead><tr><th>Team</th><th>Points</th><th>Note</th><th>Entered By</th></tr></thead><tbody>
        ${day.arbitraryAwards.map((a) => `
          <tr>
            <td style="font-weight:700">${escapeHtml(teamDisplayName(a.team))}</td>
  ...
  if (cat === "excused") {
    ...
    html += `
      ...
      <div style="margin-bottom:16px"><p class="section-label">Roster size (adjust if a team's real headcount changes)</p>
        <table><thead><tr><th>Team</th><th>Roster</th></tr></thead><tbody>
          ${TEAMS.map((team) => `
            <tr>
              <td style="font-weight:700">${teamDisplayName(team)}</td>
    ...
    TEAMS.forEach((team) => {
      ...
      html += `
        <div class="team-block">
          <p class="section-label">${teamDisplayName(team)} - ${excusedList.length} excused (${excusedWindow})</p>
...
// Every mutation goes through mutateDayTransactional(), which reads the
// CURRENT server data, applies the intended change, and writes it back —
// all inside a single Firestore transaction. If someone else's write lands
// in between, Firestore automatically re-runs the whole callback against
// the newer data and tries again, transparently, with no lost work and no
// alert shown to anyone. This matters a lot at real scale: with many RSS
// checking in within seconds of each other, near-simultaneous writes to
// the same day document are the NORMAL case, not a rare edge case — an
// earlier version of this function detected that case and just discarded
// the change with a warning, which silently dropped real check-ins under
// exactly the load a live session produces. Reapplying the same small,
// well-defined change (append a record, toggle a flag, adjust a count)
// against fresh data is safe and correct here; it's not the same as
// picking a winner between two genuinely conflicting edits.
async function mutateDayTransactional(dayIndex, mutatorFn) {
  const { db, runTransaction } = window.__firebase;
  const ref = dayDocRef(dayStorageKey(dayIndex));
  let finalDay = null;
  let finalRev = null;
  ...
  await runTransaction(db, async (tx) => {
    ...
    const { __rev, ...cleanRemote } = remote || {};
    const baseDay = remote ? cleanRemote : makeEmptyDay(dayIndex);
    mutatorFn(baseDay);
    finalRev = (__rev || 0) + 1;
    finalDay = baseDay;
    tx.set(ref, { ...baseDay, __rev: finalRev });
  });

  const idx = state.days.findIndex((d) => d.dayIndex === dayIndex);
  if (idx !== -1) state.days[idx] = finalDay;
  else state.days.push(finalDay);
  state.dayRevisions[dayIndex] = finalRev;
  mirrorDayBackup(dayStorageKey(dayIndex), { ...finalDay, __rev: finalRev }); // fire-and-forget, unchanged from before
  return finalDay;
}

// Like mutateDayTransactional, but specifically for adding an attendance
// record — the duplicate-check and excused-check happen INSIDE the same
// transaction as the write itself, not as a separate step beforehand. This
// closes a real gap the simpler pre-check-then-write pattern had: two
// near-simultaneous submissions could each read "not a duplicate yet"
// before either had written, and both get recorded. Checking and writing
// atomically together means only one can ever win.
async function attemptAddAttendanceRecord(dayIndex, name, team, sessionWindow, tier, minutesEarly) {
  const { db, runTransaction } = window.__firebase;
  const ref = dayDocRef(dayStorageKey(dayIndex));
  let outcome = null;
  let finalDay = null;
  ...
  await runTransaction(db, async (tx) => {
    ...
    const existingRecords = baseDay.attendanceRecords[sessionWindow] || [];
    const alreadyCheckedIn = existingRecords.some((r) => r.team === team && r.name.toLowerCase() === name.toLowerCase());
    if (alreadyCheckedIn) { outcome = "duplicate"; return; }

    const excusedList = (baseDay.excused[sessionWindow] && baseDay.excused[sessionWindow][team]) || [];
    if (excusedList.includes(name)) { outcome = "excused"; return; }

    baseDay.attendanceRecords[sessionWindow].push({
  ...
  if (outcome === "success") {
    ...
    state.dayRevisions[dayIndex] = finalRev;
    mirrorDayBackup(dayStorageKey(dayIndex), { ...finalDay, __rev: finalRev });
  }
  return outcome;
}

async function addAttendanceRecord(name, team, sessionWindow, tier, minutesEarly) {
  // Used only by addManualAttendanceRecord and similar admin-initiated
  // paths where SRSS themselves are choosing the person from a filtered
  // dropdown that already excludes existing records — the race window that
  // matters for real RSS check-ins (see attemptAddAttendanceRecord above)
  // isn't a practical concern there.
  const dayIndex = today().dayIndex;
  await mutateDayTransactional(dayIndex, (day) => {
...
async function deleteAttendanceRecord(sessionWindow, team, index, reason) {
  const dayIndex = today().dayIndex;
  await mutateDayTransactional(dayIndex, (day) => {
    const teamRecords = day.attendanceRecords[sessionWindow].filter((r) => r.team === team);
    const target = teamRecords[index];
    ...
    const globalIndex = day.attendanceRecords[sessionWindow].indexOf(target);
    if (globalIndex === -1) return;
    if (!day.deletedRecordsLog) day.deletedRecordsLog = [];
    day.deletedRecordsLog.push({
      team, window: sessionWindow, deletedRecord: { ...target }, reason, timestamp: Date.now(),
    });
...
async function toggleExcused(sessionWindow, team, person) {
  const dayIndex = today().dayIndex;
  let wasAlreadyExcused = false;
  const finalDay = await mutateDayTransactional(dayIndex, (day) => {
    const list = day.excused[sessionWindow][team];
    const idx = list.indexOf(person);
    wasAlreadyExcused = idx >= 0;
    if (wasAlreadyExcused) list.splice(idx, 1);
    else list.push(person);
  });
  debugLog(`excused list for ${team}/${sessionWindow} now: ${JSON.stringify(finalDay.excused[sessionWindow][team])}`);
  // Only relevant when newly marking someone excused (not un-excusing), and
  // only if they already have an attendance record this exact session —
  // same conflict already flagged visually in Attendance Records. Checked
  // against the FINAL committed data, not a value from a possibly-retried
  // attempt, and the webhook call itself lives outside the transaction so
  // it can never fire more than once for one logical toggle.
  if (!wasAlreadyExcused) {
    const hasRecord = (finalDay.attendanceRecords[sessionWindow] || []).some((r) => r.team === team && r.name.toLowerCase() === person.toLowerCase());
    if (hasRecord) {
      sendExceptionAlert("excused-after-checkin", { name: person, team: teamDisplayName(team), window: sessionWindow, day: finalDay.label, detail: "Marked excused, but already has a check-in record this session — resolve which is correct" });
    }
  }
  render();
}
...
// Creates a new day in DRAFT status — fully configurable (session times,
// session names, everything) but NOT what public check-in points to. Safe
// to do at any point ahead of time without affecting today's live check-in
// at all.
async function prepNextDay() {
  const currentLive = liveDay();
  const newIndex = state.days.length > 0 ? Math.max(...state.days.map((d) => d.dayIndex)) + 1 : 1;
  // Carry forward whatever roster sizes were in effect on the current live
  // day — so a correction SRSS made (e.g. a team's real headcount changing
  // mid-competition) persists automatically instead of silently reverting
  // to the original hardcoded defaults every time a new day is prepped.
  const newDay = makeEmptyDay(newIndex, currentLive ? currentLive.rosterSizes : null, "draft");
  state.days.push(newDay);
  const indices = state.days.map((d) => d.dayIndex);
  await saveDay(dayStorageKey(newIndex), newDay);
  state.dayRevisions[newIndex] = 0;
  await saveDayIndex(indices);
  state.viewingDayIndex = newDay.dayIndex; // jump to viewing/configuring the new draft day
  render();
  return newDay;
}

// Switches which day is actually live — closes out whichever day was
// previously live (snapshot + marked closed, same as before) and flips the
// target day (usually a day already prepped as a draft) to live. This is
// the moment public check-in actually starts flowing to the new day.
async function goLiveWithDay(dayIndex) {
  const target = state.days.find((d) => d.dayIndex === dayIndex);
  if (!target) return;
  const currentLive = liveDay();

  if (currentLive && currentLive.dayIndex !== dayIndex) {
    // Freeze an immutable copy of the day that's ending, exactly as it
    // stood at close — this is the point-in-time backup, separate from the
    // live mirror that updates on every save.
    await snapshotDayClose(currentLive);
    await mutateDayTransactional(currentLive.dayIndex, (day) => { day.status = "closed"; });
  }
  await mutateDayTransactional(dayIndex, (day) => { day.status = "live"; });
  state.viewingDayIndex = dayIndex; // jump to viewing the newly-live day
  render();
}

// The original one-click behavior: prep a new day AND make it live
// immediately, for whenever there's no reason to plan ahead.
async function startNewDay() {
  const newDay = await prepNextDay();
  await goLiveWithDay(newDay.dayIndex);
}
...
function exportToExcel() {
  ...
  state.days.forEach((day) => {
    ...
    day.themeDay.forEach((row) => {
      const excusedSet = excusedForThemeDay(day, row.team);
      const themeRoster = effectiveRosterCount(day.rosterSizes[row.team], excusedSet.size);
      const onThemeCount = effectiveOnThemeCount(row.onTheme, excusedSet);
      const themePool = themeDayPoolFor(onThemeCount, themeRoster);
      const themePts = Math.round((themePool / themeRoster) * 10) / 10;
      row.onTheme.forEach((entry) => {
        const isExcusedConflict = excusedSet.has(entry.name);
        logRows.push({
          Day: day.label, Team: row.team, Category: "Theme Day", Points: isExcusedConflict ? 0 : themePts,
          Note: isExcusedConflict
            ? `${entry.name} marked on-theme but also excused today — not counted toward points`
            : `${entry.name} dressed on-theme (logged ${new Date(entry.timestamp).toLocaleTimeString()})`,
          "Entered By": "SRSS",
        });
      });
    });
  ...
  const deletedRows = [];
  state.days.forEach((day) => {
    (day.deletedRecordsLog || []).forEach((entry) => {
      deletedRows.push({
        Day: day.label, Team: teamDisplayName(entry.team), Window: entry.window,
        "Deleted Person": entry.deletedRecord.name, "Original Tier": entry.deletedRecord.tier,
        Reason: entry.reason, "Deleted At": new Date(entry.timestamp).toLocaleString(),
      });
    });
  });
  const ws3b = XLSX.utils.json_to_sheet(deletedRows.length > 0 ? deletedRows : [{ Day: "", Team: "", Window: "", "Deleted Person": "", "Original Tier": "", Reason: "No records deleted", "Deleted At": "" }]);
  ws3b["!cols"] = [{ wch: 8 }, { wch: 16 }, { wch: 8 }, { wch: 18 }, { wch: 12 }, { wch: 40 }, { wch: 18 }];
  XLSX.utils.book_append_sheet(wb, ws3b, "Deleted Records");
...
function wireAll() {
  ...
  // Prep Next Day — creates a draft day to configure ahead of time. No
  // export needed here since nothing is closing yet.
  const prepNextDayBtn = document.getElementById("prep-next-day-btn");
  if (prepNextDayBtn) {
    prepNextDayBtn.onclick = () => {
      prepNextDay().then(() => attachDayListenerForCurrentView());
    };
  }

  // Go Live With This Day — switches the currently-viewed (draft) day to
  // live, closing out whatever was live before. Same export-before-closing
  // reasoning as Start New Day above, since this is also a closing moment.
  const goLiveBtn = document.getElementById("go-live-btn");
  if (goLiveBtn) {
    goLiveBtn.onclick = () => {
      const targetIndex = today().dayIndex; // the draft day currently being viewed
      try { exportToExcel(); } catch (e) { debugLog(`auto-export on go-live failed: ${e.message || e}`); }
      goLiveWithDay(targetIndex).then(() => attachDayListenerForCurrentView());
    };
  }
  ...
  const toggleLeaderboardBtn = document.getElementById("toggle-leaderboard-visibility-btn");
  if (toggleLeaderboardBtn) {
    toggleLeaderboardBtn.onclick = () => {
      const goingHidden = !state.leaderboardHidden;
      if (goingHidden && !window.confirm("Hide the leaderboard from RSS? They'll see a placeholder instead of standings until you reveal it again.")) return;
      setLeaderboardHidden(goingHidden);
    };
  }
  ...
  document.querySelectorAll("[data-save-record]").forEach((el) => {
    el.onclick = () => {
      ...
      if (!notesInput.value.trim()) {
        window.alert("Please add a short note explaining this change before saving — this is the only record of why an attendance edit was made.");
        return;
      }
      editAttendanceRecord(w, team, Number(indexStr), { tier: tierSelect.value, notes: notesInput.value.trim() });
  ...
  document.querySelectorAll("[data-delete-record]").forEach((el) => {
    el.onclick = () => {
      ...
      const [w, team, indexStr] = key.split("|");
      const reason = window.prompt(`Why are you deleting this record for ${teamDisplayName(team)}? Include your name — this is logged and can't be recovered otherwise.`);
      if (!reason || !reason.trim()) {
        window.alert("A reason is required to delete a record — nothing was deleted.");
        return;
      }
      deleteAttendanceRecord(w, team, Number(indexStr), reason.trim());
      state.editingRecord = null;
    };
  });
...
function wireCheckInForm() {
  // Used only by the admin "Test the check-in flow" screen — a real QR scan
  // already carries a timeKey via the deep link, so this only matters when
  // team/window are picked manually. Without this, admin-tested check-ins
  // never know which specific time-group they represent, so sessionName
  // (and anything else keyed by timeKey) would always come through blank
  // even when a real session name was set in Session Setup.
  function refreshManualTimeKey() {
    if (checkinFormState.deepLinkTeams) return; // real QR scan already has a valid timeKey — don't override it
    const day = today();
    const config = day.sessionConfig[checkinFormState.window];
    const teamTime = config && checkinFormState.team ? config.teamTimes[checkinFormState.team] : null;
    checkinFormState.deepLinkTimeKey = teamTime ? `${teamTime.hour}:${teamTime.minute}` : null;
  }

  const windowSelect = document.getElementById("checkin-window");
  if (windowSelect) windowSelect.onchange = () => { checkinFormState.window = windowSelect.value; refreshManualTimeKey(); render(); };
