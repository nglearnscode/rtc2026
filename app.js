...
function calculateCumulativeTotals(days) {
  ...
  days.forEach((day) => {
    ...
    day.themeDay.forEach((row) => {
      ...
      const pool = themeDayPoolFor(row.onTheme.length, roster);
      totals[row.team].themeDay += row.onTheme.length * (pool / roster);
    });
...
function renderManualEntryHtml(day) {
  ...
  if (cat === "theme-day") {
    ...
    day.themeDay.forEach((row) => {
      ...
      const pts = Math.round(row.onTheme.length * (pool / roster));
      html += `
...
function exportToExcel() {
  ...
  state.days.forEach((day) => {
    ...
    day.themeDay.forEach((row) => {
      ...
      const themePool = themeDayPoolFor(row.onTheme.length, themeRoster);
