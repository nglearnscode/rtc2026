...
const SAMPLE_ROSTERS = {
  ...
  Central: ["To be announced", "Hoor Atique", "Ximena Castillo Ramirez", "Ava Jalali", "Julia Kary", "Oona Kauppinen", "Rachel Rui Liang", "Siddharth Naik", "Daphne Nguyen", "Mitaansh Niverthi", "Areesh Noman", "Senad Sadik", "Kaitlyn Stevens", "Jacelynn Su", "Krish Thakur"],
...
// Two or more SRSS could plausibly be in the admin dashboard on the same
// day's data at once. Since each day is one Firestore document, a naive
// "read into memory, edit, write the whole thing back" pattern means
// whoever saves LAST wins — silently discarding the other person's change,
// with no error, no warning, nothing. saveDayTransactional() below closes
// that gap: every save first checks, atomically, whether the document has
// been modified by someone else since this device last saw it. If so, the
// save is refused and the newer data is pulled in instead of being
// clobbered — the person just needs to redo their edit against current
// data, rather than losing it without ever knowing.
async function saveDayTransactional(day) {
  const { db, runTransaction } = window.__firebase;
  const ref = dayDocRef(dayStorageKey(day.dayIndex));
  const knownRev = state.dayRevisions[day.dayIndex] || 0;
  let resultRev = null;
  ...
  await runTransaction(db, async (tx) => {
    ...
    const remoteRev = remote ? (remote.__rev || 0) : 0;
    if (remote && remoteRev !== knownRev) {
      conflictData = remote; // someone else saved since we last loaded this day
      return;
    }
    resultRev = remoteRev + 1;
    tx.set(ref, { ...day, __rev: resultRev });
  });

  if (conflictData) {
    const err = new Error("Concurrent edit conflict — someone else saved this day since it was last loaded here.");
    err.__conflict = true;
    err.__remoteData = conflictData;
    throw err;
  }
  state.dayRevisions[day.dayIndex] = resultRev;
  mirrorDayBackup(dayStorageKey(day.dayIndex), { ...day, __rev: resultRev }); // fire-and-forget, unchanged from before
}

async function persistToday() {
  const day = today();
  try {
    await saveDayTransactional(day);
  } catch (e) {
    if (e.__conflict) {
      debugLog(`Save conflict on day ${day.dayIndex} — another device saved first; reloading latest data, your last change was NOT saved.`);
      applyRemoteDayData(e.__remoteData);
      window.alert("Someone else just updated this day at the same moment, so your last change wasn't saved. The screen now shows their latest version — please redo your edit.");
      render();
    } else {
      console.error("Failed to save day:", e);
      debugLog(`persistToday FAILED: ${e.message || e}`);
    }
  }
}
...
async function toggleExcused(sessionWindow, team, person) {
  ...
  // same conflict already flagged visually in Attendance Records.
  if (!wasAlreadyExcused) {
    const hasRecord = (day.attendanceRecords[sessionWindow] || []).some((r) => r.team === team && r.name.toLowerCase() === person.toLowerCase());
    if (hasRecord) {
      sendExceptionAlert("excused-after-checkin", { name: person, team: teamDisplayName(team), window: sessionWindow, day: day.label, detail: "Marked excused, but already has a check-in record this session — resolve which is correct" });
    }
  }
  await persistToday();
  render();
