...
const SAMPLE_ROSTERS = {
  Northside: ["Juna Abutaha", "Suryansh Baichoo", "Cielo Barrera Amacifuen", "Sofia Benard", "Xavier Blache", "Saif Cheema", "Rayanna Clarke", "Mackenzie Geith", "Javonte Hunter", "Eilin Kulinich", "Sonakshi Mitra", "Georgina Murray", "Adegheosa Omorogiuwa", "Shahzaib Sheikh", "Khue Tran", "Aisling MacQuarrie", "Maria Paula Garzon"],
  Southside: ["Amelia Alam", "Spencer Brodie", "Dilni Dissanayake", "Jenny Dong", "Irha Hassan", "Munkhzul Jargalsaikhan", "Tishma Joarder", "Farhada Khaled", "Mairin McConnell", "Helya Rassouli", "Karisa Sol-Edeigba", "Kels Themens", "Raisha Valezka", "Ji-in Yun"],
  Central: ["Divine Aine", "Hoor Atique", "Ximena Castillo Ramirez", "Ava J. Jalali", "Julia Kary", "Oona Kauppinen", "Rachel Rui Liang", "Siddharth Naik", "Daphne Nguyen", "Mitaansh Niverthi", "Areesh Noman", "Senad Sadik", "Kaitlyn Stevens", "Jacelynn Su", "Krish Thakur"],
  NRB: ["Katia Al Shehadeh", "Riana Banks", "Ava B. Boston", "Aryan Garg", "Tia Girolametto", "Sabea Larson", "Sarah McGinn", "Presley McLeod", "Emma-Marie Meyer", "Giulia Nuvoloni", "Semilore Olafare", "Princess O. (Don) Olawale", "Sreyasi Rahman", "Aurora Thompson", "Gio Williams"],
  RSA: ["Divva Achpilya", "Reese Cameron", "MJ Comuzzi", "Olivia Dalla Zanna", "Avery Harvey", "Madeline Kelly", "Heeje Kim", "Karina Kosareva", "Kartika Kumala", "Hamad Malik", "Ghazlia Mehdi", "Lawrene Naval", "Michelle Omitayo", "Fehintoluwa Omotunde", "Princess O. (RSA) Owusu Ansah", "Blake Pederson", "Matthew Pichocki", "Krisha Shah", "Darsh Singh", "Natran Tewoldemedhin", "Kayla Viveiros", "Akaila Wright"],
  REF: ["Vera Allue", "Amina Djibrine Sy", "Elif Ekizoglu", "Josie Fratarcangeli", "Ana Giceva", "Saba Halabisaz", "Winston Han", "Adriana Izilein", "Aashritha Kadiri", "Harshita Kakkar", "Saurabh Nair", "Demi O. Ogundele", "WEEDAD Okpala", "Olamide Olajide", "Gabrielle Omoyinbo", "Surya Pusapati", "Emily Savoie", "Nola Ryissa Scott", "Xin Wren Shan", "Muhammad Umar", "Rohan Verma"],
  PF: ["Demi A. Akinleye-Abraham", "Marcus Anastacio", "Lucy Chen Chen", "Iris Cioban", "Elias Perianza Robles", "Aizhanym Shaikhiyeva", "Zahra Simpson-Stairs", "Samantha Venegas Guillen"],
};
...
function renderQRVisual(sessionWindow, timeKey) {
  ...
  try {
    ...
    const qr = qrcode(0, "M"); // type 0 = auto-detect size, M = medium error correction
    qr.addData(deepLink);
    qr.make();
...
async function boot() {
  try {
    ...
    if (checkinFormState.deepLinkTeams) state.publicTab = "checkin";
    render();
  } catch (e) {
...
function parseCheckInDeepLink() {
  ...
  if (!sessionWindow || !timeKey) return;
  const day = today();
  ...
  if (!teamsInGroup) return; // stale/invalid link — falls back to manual mode
    ...
    if (records.length === 0) {
      ...
      records.forEach((rec, i) => {
        ...
        if (isEditing) {
          ...
          html += `
            <tr ${isSuspicious ? 'style="background:#fff0ed"' : ""}>
              <td style="font-weight:700">${escapeHtml(rec.name)} ${isSuspicious ? `<span title="Checked in ${rec.minutesEarly} min before start — worth a look" style="color:#b3422a;font-weight:700">⚠ ${rec.minutesEarly}m early</span>` : ""}</td>
              <td>${rec.tier}</td>
              <td>${new Date(rec.timestamp).toLocaleTimeString()}</td>
